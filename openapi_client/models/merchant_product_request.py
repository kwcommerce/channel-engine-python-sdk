...
class MerchantProductRequest(BaseModel):
    ...
    """ # noqa: E501
    parent_merchant_product_no: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=64)]] = Field(default=None, description="If this product is a different version of another  product (for example, all fields are the same except  size), then this field should contain  the 'MerchantProductNo' of the parent. The parent  should already exist (or be present between the products  in the content of the API call, it does not matter whether  the parent is behind the child in the list).", alias="ParentMerchantProductNo")
    parent_merchant_product_no2: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=64)]] = Field(default=None, description="If this product is a different version of another  product (for example, all fields are the same except  color) and itself is a parent with child products (e.g. of sizes),  then this field should contain the 'MerchantProductNo' of the grandparent. The grandparent  should already exist (or be present between the products  in the content of the API call, it does not matter whether  the grandparent is behind the child in the list).  When you set this field, the ParentMerchantProductNo should be left empty.                Use this field in case of three level product hierarchy,  e.g. model - color - size.  This is required for channels like Otto.", alias="ParentMerchantProductNo2")
    extra_data: Optional[List[MerchantProductExtraDataItemRequest]] = Field(default=None, description="An optional list of key-value pairs containing  extra data about this product. This data can be  sent to channels or used for filtering products.", alias="ExtraData")
    ...
    __properties: ClassVar[List[str]] = ["ParentMerchantProductNo", "ParentMerchantProductNo2", "ExtraData", "Name", "Description", "Brand", "Size", "Color", "Ean", "ManufacturerProductNumber", "MerchantProductNo", "Stock", "Price", "MinPrice", "MaxPrice", "MSRP", "PurchasePrice", "VatRateType", "ShippingCost", "ShippingTime", "Url", "ImageUrl", "ExtraImageUrl1", "ExtraImageUrl2", "ExtraImageUrl3", "ExtraImageUrl4", "ExtraImageUrl5", "ExtraImageUrl6", "ExtraImageUrl7", "ExtraImageUrl8", "ExtraImageUrl9", "IsFrozen", "CategoryTrail"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "ExtraData": [MerchantProductExtraDataItemRequest.from_dict(_item) for _item in obj["ExtraData"]] if obj.get("ExtraData") is not None else None,
            "Name": obj.get("Name"),
